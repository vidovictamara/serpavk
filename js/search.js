...
function loadRecipe(filename, displayName) {
    let currentRecipe = "";
    let url = "recipes/" + filename;

    document.getElementById("fileName").innerHTML = displayName;
    document.getElementById("searchstat").innerHTML = "";
    document.getElementById("keyword").value = "";

    //server request to download recipe
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState == 4 && xhr.status == 200) {
            currentRecipe = xhr.responseText;


            //break
            currentRecipe = currentRecipe.replace(/(?:\r\n|\r|\n)/g, '<br>');

            document.getElementById("fileContent").innerHTML = currentRecipe;
